...
const Link = styled.div`
  ...
  color: ${(p: LinkProps): string =>
    p.active ? colours.primary : colours.dark};
  font-weight: 400;

  & > div {
    background: ${(p: LinkProps): string =>
      p.active ? colours.primary : colours.dark};
  }
...
const Contact: React.FC = () => {
  ...
  const handleSubmit = (event: any) => {
    ...
    emailjs
      .send("gmail", "NR_Contact", returnData, "user_qVvmKovzsj8m5O2fySEw6")
      .then(
        (result) => {
          console.log(result.text);
          setOverlayOpen(true);
          setOverlayText(
            "Form Successfully submitted! We will be in touch as soon as possible"
          );
        },
        (error) => {
          ...
          setOverlayOpen(true);
          setOverlayText(
            "Oops, an error occured. Please try again later, or contact us if the issue persists."
          );
        }
      );
  ...
  return (
    <Section background={colours.primary}>
      <Container>
        ...
        <SubHeader>
          Please get in touch today if you have any questions, are interested in
          any of our services - or are looking for hassle free solutions to your
          removal needs!
        </SubHeader>
